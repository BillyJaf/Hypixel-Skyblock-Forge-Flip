...
const ItemCard: React.FC<ItemCardProps> = ({ forgeItem }: ItemCardProps) => {
  ...
  const productPrice =
    filters.sellTo === 'Sell Offer'
      ? readableItemPrice(forgeItem, filters)
      : readableItemPrice(forgeItem, filters);
  ...
  const sellProduct =
    forgeItem.whereToSell === 'bazaar'
      ? `Product ${filters.sellTo}:`
      : `Product LBIN:`;
  ...
  return (
    <Box sx={itemCardMainBoxStyle}>
      <Box sx={itemCardHeaderBoxStyle}>
        <img
          src={`/items/${forgeItem.imageName}.webp`}
      ...
      <ItemCardData
        title={`Ingredients ${filters.buyFrom}:`}
