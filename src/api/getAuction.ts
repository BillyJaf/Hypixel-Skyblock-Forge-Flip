...
import { auctionItemLBINs, forgeableAuctionItems } from "../ui/utils/itemGroups";
import { armourReforges, drillReforges, equipmentReforges } from "../ui/utils/reforges";
    ...
    if (parsedData.success) {
      parsedData.auctions.forEach((auction: Auction) => {
        if (auction.bin) {
          ...
          if (forgeableAuctionItems.includes(displayName) || forgeAuctionIngredients.includes(displayName)) {
...
const isReforged = (item_name: string) => {
   if (item_name.includes("Helmet") || item_name.includes("Chestplate") || item_name.includes("Leggings") || item_name.includes("Boots")) {
   ...
   else if (item_name.includes("Belt") || item_name.includes("Cloak") || item_name.includes("Gauntlet") || item_name.includes("Necklace") || item_name.includes("Pendant")) {
